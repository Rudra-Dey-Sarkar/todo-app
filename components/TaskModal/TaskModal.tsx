...
function TaskModal({setTaskModal, taskId, dependancy, setDependancy }: {setTaskModal: React.Dispatch<React.SetStateAction<boolean>>, taskId: string, dependancy: boolean, setDependancy: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  return (
    <div className='bg-green-300 w-[40vw] h-[100vh]'>
        ...
        <div
          ...
          <div className='flex justify-between px-3 border-b-2 border-[#35793729] hover:bg-[#35793729]'>
            <div className='flex justify-between gap-x-3 w-full h-full'>
              ...
              onClick={() => EditTaskStatusData(task?._id, task?.status, dependancy, setDependancy, dep, setDep)}>
                {task?.status === false ?
                  <svg
              ...
              <p className='w-full h-full py-3 hover:cursor-pointer'>{task?.taskName}</p>
            </div>
            <button 
            disabled={pathname==="/all-tasks"}
            onClick={() => EditTaskImportantData(task?._id, task?.important, dependancy, setDependancy, dep, setDep)}>
